    ...
    <main class="mx-auto w-full sm:max-w-3xl px-2 sm:px-6 py-10">
        ...
        <div class="card-mobile sm:w-auto md:min-w-xl mx-auto overflow-hidden rounded-3xl border border-white/10 bg-white/5 shadow-2xl shadow-black/40"
            ...
            <div class="px-5 pb-6 text-center">
                ...
                @if($user->position)
                <p class="text-base text-white/80 drop-shadow-sm mt-1">{{ $user->position }}</p>
                @endif
                ...
                <div class="flex flex-col gap-2 w-full mx-auto">
                    ...
                    @foreach($links as $item)
                        <a href="{{ $item['url'] }}" target="_blank" class="flex items-center gap-4 bg-white/10 backdrop-blur-sm px-4 py-3 rounded-xl border border-white/20 text-gray-100 text-lg hover:bg-white/20 hover:text-white transition">
                            {!! $item['icon'] !!}
